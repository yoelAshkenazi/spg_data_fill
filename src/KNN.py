...
class KNN:
    _input_types = [np.ndarray, torch.Tensor]
    ...
    def _calc_knn_obj(self, data: np.ndarray, train_mask: list):
        ...
        if self.dist_name == "heur_dist":
            ...
            f = lambda r: np.sqrt(np.power(data_train - r, 2).sum(1))  # this returns an array of distances from each
