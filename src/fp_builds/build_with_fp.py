...
# use the algorithm to impute the missing values.
def impute(data) -> pd.DataFrame:
    """takes a dataframe and imputes the missing values using the GFP algorithm."""
    # initialize the GFP algorithm.
    gfp = GFP(iters=50, eps=1e-3, early_stop=False)

    # converts the dataframe to a graph and runs the GFP algorithm on it.
    knn_model = KNN()
    edges, distances = knn_model.get_edges(data)
    # impute the missing values.

    imputed_data = gfp.prop(data, edges)
